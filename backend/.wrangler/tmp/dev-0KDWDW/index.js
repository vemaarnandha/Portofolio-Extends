...
// .wrangler/tmp/bundle-O8cNHj/strip-cf-connecting-ip-header.js
...
function getServiceSupabase(env) {
  return createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE, {
    auth: { persistSession: false }
  });
}
__name(getServiceSupabase, "getServiceSupabase");
...
var IMAGE_ALLOWED_MIMES = ["image/jpeg", "image/png", "image/webp", "image/gif"];
...
__name(getImageExtension, "getImageExtension");
function generateImagePath(projectId, mime) {
  const ext = getImageExtension(mime);
  const timestamp = Date.now();
  const randomId = Math.random().toString(36).substring(2, 10);
  return `projects/${projectId}/${timestamp}-${randomId}.${ext}`;
}
__name(generateImagePath, "generateImagePath");
...
auth.post("/login", async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data: users, error } = await supabase.from("admin_users").select("*").eq("email", email).limit(1);
    if (error)
      throw error;
    const user = users?.[0];
    if (!user) {
...
portfolioRoutes.post("/upload-image", authMiddleware, async (c) => {
  try {
    ...
    const { data, error } = await supabase.storage.from("project-images").upload(filePath, arrayBuffer, {
...
portfolioRoutes.delete("/:id/image", authMiddleware, async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data: project, error: fetchError } = await supabase.from("portfolio").select("photo_url").eq("id", id).limit(1);
    ...
    if (storageIndex !== -1) {
      ...
      await serviceSupabase.storage.from("project-images").remove([storagePath]);
    }
    const { error: updateError } = await supabase.from("portfolio").update({ photo_url: null }).eq("id", id);
    if (updateError)
      throw updateError;
...
portfolioRoutes.get("/:id", async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data, error } = await supabase.from("portfolio").select("*").eq("id", id).limit(1);
    if (error)
      throw error;
...
portfolioRoutes.put("/:id", authMiddleware, async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data, error } = await supabase.from("portfolio").update({
      nama_project: body.nama_project,
      photo_url: body.photo_url,
      jobdesk: body.jobdesk,
      deskripsi: body.deskripsi
...
portfolioRoutes.delete("/:id", authMiddleware, async (c) => {
  try {
    ...
    const supabase = getSupabase(c.env);
    const { data, error } = await supabase.from("portfolio").delete().eq("id", id).select().single();
    if (error) {
      if (error.code === "PGRST116") {
        return c.json({ success: false, data: null, message: "Portfolio tidak ditemukan" }, 404);
      }
...
// .wrangler/tmp/bundle-O8cNHj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-O8cNHj/middleware-loader.entry.ts
