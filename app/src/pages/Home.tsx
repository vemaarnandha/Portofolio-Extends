  ...
  if (error) {
    return (
      <div className="absolute inset-0 w-full h-full bg-void-800 flex items-center justify-center opacity-30">
        <span className="text-xs text-arcane-500 font-body">Gambar tidak tersedia</span>
      </div>
    );
  }
...
export default function Home() {
  const bentoSkills = [
    ...
    {
      title: "Database",
      desc: "Supabase & PostgreSQL untuk integritas data yang andal.",
      icon: Database,
      color: "rift",
    ...
    },
    {
      ...
      desc: "Core Web Vitals & SEO adalah prioritas utama setiap proyek.",
      icon: Zap,
      color: "gold",
    ...
    <div className="min-h-screen">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-arcane-800/50 bg-arcane-950/40 backdrop-blur-sm px-4 py-1.5 text-xs font-body tracking-wider text-arcane-400 mb-8 animate-fade-from-abyss uppercase">
            <Sparkles className="h-3.5 w-3.5 text-enchant-400 animate-pulse" /> Siap Berkolaborasi
          </div>

          <h1 className="font-heading text-4xl sm:text-6xl lg:text-8xl font-bold tracking-tighter mb-8 animate-fade-from-abyss">
            Membangun Digital <br />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 animate-fade-from-abyss [animation-delay:400ms]">
            <Link
              to="/projects"
              className="group relative inline-flex items-center gap-2 rounded-xl bg-arcane-500 px-8 py-4 text-sm font-body tracking-wider font-bold text-void-950 overflow-hidden transition-all duration-300 hover:bg-arcane-400 hover:shadow-arcane active:scale-95"
            >
              <span className="relative z-10 flex items-center gap-2">
                Lihat Proyek Saya <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </span>
            ...
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 rounded-xl border border-arcane-800 bg-void-950/50 backdrop-blur-md px-8 py-4 text-sm font-body tracking-wider font-bold text-arcane-300 hover:bg-arcane-900/30 hover:border-arcane-600 transition-all duration-300 active:scale-95"
            >
              Hubungi Saya
            </Link>
          </div>
              ...
              <div className="relative z-10 p-6 flex flex-col justify-end h-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {bentoSkills[0].tags?.map(tag => (
                    <span key={tag} className="text-xs font-mono bg-arcane-900/50 border border-arcane-800 px-2 py-1 rounded text-arcane-400">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
